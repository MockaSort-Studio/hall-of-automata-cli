...
export function registerPullRequestTools(pi) {
  ...
  tool("github_pull_request_view", "Read a pull request.", obj({ repo: S(), pullNumber: I() }), (x) =>
    pulls.viewPullRequest(x.repo, x.pullNumber),
  );
  ...
  tool(
    ...
    "Edit pull request title, body, base branch, or state.",
