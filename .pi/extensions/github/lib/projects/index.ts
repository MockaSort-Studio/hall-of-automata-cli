...
export function getFieldMap(org, project) {
  const data = ghJson(["project", "field-list", String(project), "--owner", org, "--format", "json"]);
  const map = {};
...
export function findItemId(org, project, issueNumber) {
  const data = ghJson(["project", "item-list", String(project), "--owner", org, "--format", "json", "--limit", "500"]);
