...
export function leadRole() {
  return {
    discipline: [
      ...
      "Create one kickoff after choosing the roster. Register every created actor with crew_register before waking it; after verified agents.remove, call crew_unregister with its actor ID.",
      ...
      "In human-gated mode, every Lead tick must call crew_poll_human_requests before any closure decision. For every returned request, reply or dispatch the requested work using its threadRootId, then call crew_ack_human_requests with its comment ID. Never close or disband after your own acceptance: remain durable until GitHub reports human closure.",
    ].join(NL),
    tools: ["read", "grep", "find", "ls", "build_crew_member", "crew_kickoff", "crew_register", "crew_unregister", "crew_post", "crew_review", "crew_tell", "crew_ask", "crew_broadcast", "crew_reply", "crew_close", "crew_poll_human_requests", "crew_ack_human_requests"],
