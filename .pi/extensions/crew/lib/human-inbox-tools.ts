...
export function registerHumanInboxTools(pi) {
  pi.registerTool({
    ...
    label: "Crew: poll human requests",
    ...
    async execute(_id, input, _signal, _update, ctx) {
      ...
      return withFileMutationQueue(path, async () => {
        ...
        if (roster.status !== "started" || !roster.discussionNumber)
          throw new Error(`Crew ${input.runId} has no active Discussion`);
  ...
  pi.registerTool({
    name: "crew_ack_human_requests",
    label: "Crew: acknowledge human requests",
    description: "Lead-only durable acknowledgement after response or dispatch.",
    parameters: Type.Object({
      runId: Type.String(),
      from: Type.String(),
      commentIds: Type.Array(Type.String(), { minItems: 1 }),
    }),
    async execute(_id, input, _signal, _update, ctx) {
      const path = rosterPath(crewRoot(ctx.cwd), input.runId);
      return withFileMutationQueue(path, async () => {
        const roster = readRoster(path);
        assertLead(roster, input.from);
        const next = acknowledgeHumanRequests(roster, input.commentIds);
