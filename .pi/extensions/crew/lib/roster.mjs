...
for (const [name, automaton] of Object.entries(catalog.automata)) {
  ...
  if (!Array.isArray(automaton.domains) || automaton.domains.length < 1 || automaton.domains.length > 5 || new Set(automaton.domains).size !== automaton.domains.length || !automaton.domains.every(value => /^[a-z0-9+][a-z0-9+-]*$/.test(value))) throw new Error(`Invalid routing for automaton: ${name}`);
  if (!Array.isArray(automaton.extensions) || !automaton.extensions.every(value => typeof value === "string") || new Set(automaton.extensions).size !== automaton.extensions.length) throw new Error(`Invalid extensions for automaton: ${name}`);
  for (const extension of automaton.extensions) if (!Object.hasOwn(ARMORY, extension)) throw new Error(`Unknown Armory extension for automaton: ${name}: ${extension}`);
}
