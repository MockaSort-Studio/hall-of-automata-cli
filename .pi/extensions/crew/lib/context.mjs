...
export function minimalCrewResult(data) {
  ...
  const summary = compact(data.summary)
    .replace(/^(PASS|BLOCKED|FAIL)\s*(?::|—|-)\s*/i, "")
    .slice(0, 320);
  const record = compact(data.finalCommentUrl || data.discussionUrl);
  return [`[Crew result ${runId}] ${outcome}.`, summary, record && `Record: ${record}`].filter(Boolean).join(" ");
}

export function minimizeCrewContext(messages) {
  return messages.map((message) => {
    const data =
      message?.role === "custom" && message.customType === "pi-fabric-agent-message"
