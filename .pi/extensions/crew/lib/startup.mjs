...
export async function prepareCrew(pi, input, ctx, configDir) {
  ...
  if (
    input.resultSummaryMaxBytes !== undefined &&
    (!Number.isInteger(input.resultSummaryMaxBytes) ||
      input.resultSummaryMaxBytes < 512 ||
      input.resultSummaryMaxBytes > 50000)
  )
    throw new Error("resultSummaryMaxBytes must be an integer from 512 to 50000");
  ...
  try {
      ...
      JSON.stringify(
        {
          ...
          resultSummaryMaxBytes: input.resultSummaryMaxBytes ?? null,
          completionMode,
          leadTickTopic: completionMode === "human-gated" ? leadTickTopic : null,
          monitorIntervalMs: completionMode === "human-gated" ? monitorIntervalMs : null,
          ...repository,
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        lead,
        assignment,
        launch: launchCode(absoluteConfig),
      }),
    );
  } catch (error) {
