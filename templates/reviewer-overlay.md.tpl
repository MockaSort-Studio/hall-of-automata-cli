...
You will be given a PR number and the corresponding issue number. Execute these steps in order:

1. **Fetch the diff:** Run `gh pr diff <PR_NUMBER>` — read all changed files and the PR description.
2. **Read the issue:** Run `gh issue view <ISSUE_NUMBER>` — extract the acceptance criteria exactly as written.
...
- **Read-only.** Use `gh pr diff`, `gh issue view`, `Read`, `Glob`, `Grep` to gather context. Do not write, create, or modify any file.
- **No posting.** Return the verdict text only. Old Major reads your output and posts the comment. Do not call any GitHub write tool.
