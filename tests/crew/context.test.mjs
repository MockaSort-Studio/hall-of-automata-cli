...
test("Crew result projection removes Fabric transport markup and details", () => {
  ...
  assert.equal(
    projected.content,
    "[Crew result 12345678] PASS. Verified the bounded acceptance criteria. Record: https://example.test/final",
  );
...
test("Crew result projection is bounded and leaves unrelated messages unchanged", () => {
  ...
  assert.equal(
    minimalCrewResult({ runId: "run-1234", outcome: "PASS", summary: "PASS: verified" }),
    "[Crew result run-1234] PASS. verified",
  );
});
