...
PID=$(cat "$TMP/.hall/watcher.pid" 2>/dev/null || echo 0)
...
sleep 60 &
FAKE_PID=$!
echo "$FAKE_PID" > "$TMP2/.hall/watcher.pid"
...
(cd "$TMP2" && \
  ...
  if [ -n "$WPID" ] && ps -p "$WPID" -o comm= 2>/dev/null | grep -q watcher; then
    echo "false" > "$TMP2/.hall/_new_started"
  else
    HOME="$TMP2" POLL_INTERVAL=1 bash "$SCRIPT" --once &
    echo "true" > "$TMP2/.hall/_new_started"
  fi
)
sleep 2
check "new watcher started despite stale PID file" \
  "[ '$(cat $TMP2/.hall/_new_started 2>/dev/null)' = 'true' ]"
