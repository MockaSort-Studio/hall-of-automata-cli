...
- Schedule: `*/15 * * * *`
- Prompt: `"Autonomous plan advancement (cron): run /hall:review. If newly unlocked READY tasks exist, dispatch them without confirmation. Append one-line summary to ~/.hall/cron-log.md."`

Store the returned ID in `~/.hall/$REPO/cron.json` as `{"cron_id":"<returned ID>","created_at":"<ISO timestamp>"}`.
